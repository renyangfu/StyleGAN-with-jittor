...
class EqualLR:
    ...
    def compute_weight(self, module):
        ...
        fan_in = weight.numel() / weight.shape[0]
...
class FusedUpsample(jt.Module):
    def __init__(self, in_channel, out_channel, kernel_size, padding=0):
        ...
        weight = jt.array(np.random.normal(0, 1, (in_channel, out_channel, kernel_size, kernel_size))).float32()
    ...
    def execute(self, input):
        ...
        out = nn.conv_transpose(input, weight, self.bias, stride=2, padding=self.pad)
...
class FusedDownsample(nn.Module):
    def __init__(self, in_channel, out_channel, kernel_size, padding=0):
        ...
        weight = jt.array(np.random.normal(0, 1, (out_channel, in_channel, kernel_size, kernel_size))).float32()
        bias = jt.zeros(out_channel)
...
class BlurFunctionBackward(Function):
    ...
    def grad(self, gradgrad_output):
        kernel, kernel_flip = self.kernel, self.kernel_flip
...
class Blur(nn.Module):
    def __init__(self, channel):
        ...
        weight = jt.float32([[1, 2, 1], [2, 4, 2], [1, 2, 1]])
        weight = weight.view(1, 1, 3, 3)
        weight = weight / weight.sum()
        weight_flip = jt.flip(weight, 2)
        weight_flip = jt.flip(weight_flip, 3)

        self._weight = weight.repeat(channel, 1, 1, 1).stop_grad()
        self._weight_flip = weight_flip.repeat(channel, 1, 1, 1).stop_grad()

    def execute(self, input):
        ret = BlurFunction()(input, self._weight, self._weight_flip)
        return ret
...
class EqualConv2d(nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        conv = nn.Conv(*args, **kwargs)
        jt.init.gauss_(conv.weight)
        jt.init.constant_(conv.bias)
...
class EqualLinear(nn.Module):
    def __init__(self, in_dim, out_dim):
        ...
        jt.init.gauss_(linear.weight)
        jt.init.constant_(linear.bias)
...
class ConstantInput(nn.Module):
    def __init__(self, channel, size=4):
        super().__init__()
        self.input = jt.array(np.random.normal(0, 1, (1, channel, size, size))).float32()
...
class Generator(nn.Module):
    ...
    def requires_grad_step(self, step, flag):
        if not flag:
            for i in range(9):
                requires_grad(self.progression[i], False)
                requires_grad(self.to_rgb[i], False)
        else:
            for i in range(9 - step - 2, -1, -1):
                requires_grad(self.progression[i], True)
                requires_grad(self.to_rgb[i], True)
...
class StyledGenerator(nn.Module):
    ...
    def requires_grad_step(self, step, flag):
        self.generator.requires_grad_step(step, flag)
    ...
    def execute(
        ...
        if noise is None:
            ...
            for i in range(step + 1):
                size = 4 * 2 ** i
                noise.append(jt.init.gauss((batch, 1, size, size), "float32"))
...
def std(x, dim):
    return jt.sqrt((x-x.mean(dim=dim, keepdims=True)).sqr().mean(dim=dim).maximum(1e-6))
...
class Discriminator(nn.Module):
    ...
    def requires_grad_step(self, step, flag):
        requires_grad(self.linear, flag)
        if not flag:
            for i in range(9):
                requires_grad(self.progression[i], False)
                requires_grad(self.from_rgb[i], False)
        else:
            for i in range(self.n_layer - step - 2, -1, -1):
                requires_grad(self.progression[i], True)
                requires_grad(self.from_rgb[i], True)
    ...
    def execute(self, input, step=0, alpha=-1):
        for i in range(step, -1, -1):
            ...
            if i == 0:
                ...
                out = jt.contrib.concat([out, mean_std], 1)

            # print("Used progression", index)
            out = self.progression[index](out)

            if i > 0:
                if i == step and 0 <= alpha < 1:
                    skip_rgb = nn.pool(input, 2, "mean")
